...
export default function SecurityAuditorModal({
  isOpen,
  onClose,
  appId,
}: SecurityAuditorModalProps) {
  ...
  const startScan = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setData({
          suspiciousIPs: Array.isArray(result.suspiciousIPs)
      ...
      <div className="bg-secondary w-full max-w-4xl rounded-[2.5rem] border border-gray-800 shadow-2xl overflow-hidden shadow-black/80 flex flex-col h-[80vh]">
        <header className="p-8 border-b border-gray-800 flex justify-between items-center bg-secondary/80 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-black text-white">
                {t("dashboard.auditor_title", "Security Auditor")}
              </h3>
              <p className="text-xs text-gray-400 font-medium">
                {t(
                  "dashboard.auditor_subtitle",
                  "Automated threat detection and anomalies scan",
                )}
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="p-3 hover:bg-gray-800 rounded-2xl transition-all text-gray-500 hover:text-white cursor-pointer select-none"
          >
            <X className="w-6 h-6" />
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8 relative">
          {scanning ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-6">
              ...
              <h4 className="text-white font-black text-xl tracking-widest uppercase">
                {t("dashboard.auditor_scanning", "Scanning Database")}
              </h4>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="p-0 overflow-x-auto">
                  {!data?.suspiciousIPs || data.suspiciousIPs.length === 0 ? (
                    <div className="p-8 text-center text-gray-500 text-sm font-medium">
                      {t(
                        "dashboard.auditor_no_ips",
                        "No brute-force or massive failed attempts detected recently.",
                      )}
                    </div>
                  ) : (
                    <table className="w-full text-left">
                      <thead className="bg-dark/50">
                        <tr>
                          <th className="px-6 py-3 text-[10px] text-gray-500 uppercase font-black">
                            IP Address
                          </th>
                          <th className="px-6 py-3 text-[10px] text-gray-500 uppercase font-black">
                            Failed Attempts (7d)
                      ...
                      <tbody className="divide-y divide-gray-800/50">
                        {data.suspiciousIPs.map((ip, i) => (
                          ...
                          >
                            <td className="px-6 py-4 font-mono text-sm text-gray-300 select-all">
                              {ip.ip}
                            </td>
                            <td className="px-6 py-4 text-red-400 font-black">
                              {ip.failedAttempts}
                            </td>
              ...
              <section className="bg-dark/30 rounded-3xl border border-gray-800/50 overflow-hidden mt-8">
                <div className="p-6 border-b border-gray-800/50 flex items-center gap-3 bg-orange-500/5">
                  ...
                  <h4 className="text-white font-black">
                    {t("dashboard.auditor_keys", "Potentially Shared Keys")}
                  </h4>
                ...
                <div className="p-0 overflow-x-auto">
                  {!data?.sharedKeys || data.sharedKeys.length === 0 ? (
                    <div className="p-8 text-center text-gray-500 text-sm font-medium">
                      {t(
                        "dashboard.auditor_no_keys",
                        "No suspicious license sharing detected (Based on unique countries/IPs per key).",
                      )}
                    </div>
                  ) : (
                    <table className="w-full text-left">
                      <thead className="bg-dark/50">
                        <tr>
                          ...
                          <th className="px-6 py-3 text-[10px] text-gray-500 uppercase font-black text-right">
                            Action
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-800/50">
                        {data.sharedKeys.map((key, i) => (
                          ...
                          >
                            <td
                              className="px-6 py-4 font-mono text-sm text-gray-300 truncate max-w-[200px]"
                              title={key.licenseKey}
                            >
                              {key.licenseKey}
                            </td>
                            <td className="px-6 py-4 text-orange-400 font-black">
                              {key.ipsCount}
                            </td>
        ...
        <footer className="p-6 bg-dark/50 border-t border-gray-800 flex justify-between items-center shrink-0">
          {data && (
            <span className="text-xs text-gray-500 font-medium ml-4">
              {t(
                "dashboard.auditor_completed",
                "Scan completed. {{count}} potential issues found.",
                { count: data.suspiciousIPs.length + data.sharedKeys.length },
              )}
            </span>
          )}
          <button
          ...
          >
            ...
            {t("dashboard.auditor_rescan", "Run New Scan")}
          </button>
        </footer>
      </div>
    </div>
