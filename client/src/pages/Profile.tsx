import { CheckCircle2, Key, Mail, ShieldAlert, User } from "lucide-react";
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus({
          type: "success",
          msg: t("profile.update_success", "Profile updated successfully"),
        });
        if (data.token) {
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000 max-w-3xl mx-auto">
      ...
      <header>
        ...
        <p className="text-gray-400 mt-1 font-medium">
          {t("profile.subtitle", "Manage your personal settings and identity")}
        </p>
      </header>
      ...
      <div className="bg-secondary border border-gray-800 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden">
        {status && (
          <div
            className={`mb-8 flex items-center gap-2 px-4 py-3 rounded-2xl text-xs font-bold border animate-shake ${status.type === "success" ? "text-green-500 bg-green-500/10 border-green-500/20" : "text-red-500 bg-red-500/10 border-red-500/20"}`}
          >
            {status.type === "success" ? (
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          ...
          <div className="space-y-2 pt-4">
            <label
              ...
              htmlFor="password"
            >
              {t("profile.new_password", "New Password (Leave blank to keep)")}
            </label>
            <div className="relative group">
        ...
        <header className="mb-8">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl font-black text-white">
              {t("profile.api_keys", "Developer API Keys")}
            </h3>
          </div>
          <p className="text-gray-500 text-xs mt-1">
            {t(
              "profile.api_keys_desc",
              "Authenticate your external services (Sellix, Shoppy) via these keys.",
            )}
          </p>
        </header>
    ...
    <div className="space-y-6">
      {generatedKey && (
        <div className="bg-accent/10 border border-accent/20 p-6 rounded-2xl space-y-4 animate-in zoom-in-95 duration-500">
          <p className="text-xs font-black text-accent uppercase tracking-widest">
            {t("profile.new_key_generated", "New Secret Key Generated")}
          </p>
      ...
      <div className="flex gap-2">
        <input
          type="text"
          placeholder={t(
            "profile.key_name_placeholder",
            "Key Name (e.g. Sellix Integration)",
          )}
      ...
      <div className="space-y-2">
        {Array.isArray(keys) &&
          keys.map((key) => (
            <div
              key={key.id}
              className="bg-dark/30 p-4 rounded-2xl border border-gray-800/50 flex justify-between items-center group"
            >
              <div>
